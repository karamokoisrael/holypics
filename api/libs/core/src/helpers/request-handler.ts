...
export const customStringify = (query: Record<string, any>) => {
  return (
    Object.keys(query)
...
export const customFetchWithCredentials = (
  url: string,
...
export const customFetchWithCredentialsAndFiles = (
  url: string,
  bearerToken: string | null,
  files: Array<any>,
...
export const getConfigs = async (callback: (data: any) => void) => {
  try {
...
// export const serializeSocketMessage = (json: Record<string, any>)=> JSON.stringify(json);
...
export const uploadBase64File = (
  ...
  return new Promise((resolve, reject) => {
    ...
    form.append("file", stream, {
      filename: fileName,
      contentType: mimeType as unknown as string,
    });
    ...
    axios
      .post(
        formatUrl(
          req,
          `files?${access_token != "" ? "access_token=" + access_token : ""}`
        ),
        form,
        {
          headers: {
...
export const getTranslations = (
  req: Request,
  accessToken: string
): Promise<DirectusData<DirectusTranslations>> => {
  return new Promise((resolve, reject) => {
    axios
      .get<DirectusData<DirectusTranslations>>(
        formatUrl(
          req,
          `settings?fields[]=translation_strings&limit=-1&access_token=${accessToken}`
        )
      )
      .then((response) => {
        resolve(response.data);
...
export const translate = (
  translations: DirectusData<Record<string, any>>,
  text: string,
  lang = DIRECTUS_DEFAULT_LANGUAGE
) => {
  const foundTranslation = (translations.data.translation_strings as any).find(
    (item: Record<string, any>) => item.key == text.replace("$t:", "")
  );
