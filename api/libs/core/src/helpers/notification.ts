...
export const notify = async (
  database: Knex,
  mailService: MailService | null = null,
  subject: string,
  message: string,
  userId: string,
  ...
  try {
    const [user]: User[] = await database("directus_users")
      .where({ id: userId })
      .limit(1);
    ...
    if (emailActivated && mailService != null) {
      const mailData =
        Object.keys(data).length > 0
          ? data
          : { headerText: subject, content: message, btnText: "", btnUrl: "" };
      await sendMail(
        mailService,
    ...
    if (
      ...
      await sendSms(
        database,
        [{ phoneNumber: user.phone_number.replace("+", "") }],
        smsText == null ? message : smsText
      );
    }
    return {} as any;
  } catch (error) {
