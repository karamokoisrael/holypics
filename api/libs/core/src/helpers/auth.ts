...
import { USER_TOKEN_EXPIRATION } from "./../consts/global";
...
export const getAdminTokens = async (database: any): Promise<AdminTokens> => {
  ...
  try {
    const [[{ token, id }]] = await database.raw(
      "SELECT directus_users.token, directus_users.id from directus_roles RIGHT JOIN directus_users ON directus_users.role = directus_roles.id AND directus_users.token IS NOT NULL WHERE directus_roles.admin_access=1 LIMIT 1"
    );
    adminId = id;
    accessToken = token;
...
export const secureUserData = (
  ...
  try {
    const accessToken =
      req == null ? currentAccessToken : getUserAccessToken(req);
    return {
      data: { ...user, ...nullables, token: accessToken, ...additionalProps },
    };
  } catch (error) {
    console.log(error);
...
export const reinitAlertLevel = async (
  database: Knex,
...
export const increaseAlertLevel = async (
  database: Knex,
  userId: number | null = null,
  email: string | null = null,
  currentAlertLevel = -2
...
export const updateAlertLevel = async (
  ...
  if (alertLevel == -1) {
    ...
    alertLevel = user?.alert_level as number;
  }

  // @ts-ignore
  [alertLevel, accountLocked] = checkoutAlertLevel(alertLevel, reinit);
  await database("directus_users")
    .update({ alert_level: alertLevel, account_locked: accountLocked })
    .where(idField, "=", idValue);
};
