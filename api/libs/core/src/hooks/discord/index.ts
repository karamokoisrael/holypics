...
export default function (
  ...
  { database, emitter }: ExtendedApiExtensionContext
) {
  type DiscordSetting = {
    app_id: string;
    public_key: string;
  ...
  action("server.start", async () => {
    try {
      ...
      const [{ discord_settings }] = await database("configurations").select(
        "discord_settings"
      );
      const {
        bot_custom_id,
        store_all_predictions,
        app_id,
        bot_token,
        test_channels,
        prediction_folder,
      } = JSON.parse(discord_settings) as DiscordSetting;
      ...
      const processUrl = async (message: Message, url: string) => {
        try {
          ...
          const prediction = await axios.post(
            `${process.env.PUBLIC_URL as string}/holypics/predict`,
            reqParams,
          ...
          for (const key of Object.keys(prediction.data[0])) {
            ...
            predictionMessage += `${key}: ${Math.round(
              prediction.data[0][key] * 100
            )}% \n`;
          ...
          const utilityRow = new MessageActionRow().addComponents(
            new MessageButton()
              ...
              .setURL(
                store_all_predictions
                  ? `${process.env.PUBLIC_URL}/file/${uuid}`
                  : url
              )
              .setDisabled(false)
          );
      ...
      client.on("ready", () => {
        ...
        for (const guildId of guild_ids) {
          rest
            .put(Routes.applicationGuildCommands(app_id, guildId), {
              body: commands,
            })
            .then(() =>
      ...
      client.on("messageCreate", async (message) => {
        if (
          ...
          await processUrl(message, message.content.replace(/ /g, ""));
      ...
      client.on("interactionCreate", async (interaction) => {
        try {
          if (
            ...
            interaction.customId.startsWith(`${bot_custom_id}_class_`)
          ) {
            const idSubstring = interaction.customId.split(
              `${bot_custom_id}_class_`
            )[1];
            // @ts-ignore
            const [className, imageId] = idSubstring.split("_id_");
            await database("feedbacks")
              .update({ moderation_classes: JSON.stringify([className]) })
              // @ts-ignore
              .where("image", "=", imageId);
