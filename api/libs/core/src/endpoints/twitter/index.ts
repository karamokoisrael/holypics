...
export default function (router: Router, { database }: ApiExtensionContext) {
  ...
  const callbackURL =
    process.env.PUBLIC_URL != "/"
      ? `${process.env.PUBLIC_URL}/twitter/auth-callback`
  ...
  router.get("/auth", async (req: Request, res: Response) => {
    ...
    try {
      ...
      const { client_id, client_secret } =
        configs.twitter_settings as TwitterSetting;
  ...
  router.get("/publish", async (req: Request, res: Response) => {
    ...
    try {
      ...
      const imagePredictionData = await replicate.predict(
        "9e767fbac45bea05d5e1823f737f927856c613e18cbc8d9068bafdc6d600a0f7",
        {
